...
async function handleElement(page, parent_element, parent_bounds, element, dir) { 
    ...
    const pickedStyles = pick(styles, ['display', 'float', 'position', 'visibility', 'flexDirection', 'flex', 'marginTop', 'marginLeft', 'marginRight', 'marginBottom', 'paddingTop', 'paddingLeft', 'paddingRight', 'paddingBottom'])
    if (pickedStyles.display == 'none' || pickedStyles.visibility == 'hidden') {
        return null
    }

    // element could be anywhere!
    if (pickedStyles.position == 'absolute') {
        return null
    }
...
/*
    ...
    const client_rect = (await element.evaluate((element) => {
    ...
    if (firstChild) {
        if (layout == 'column') {
            //firstChildSize.height = firstChild.bounds.height //+ parseFloat(pickedStyles.paddingTop)
            firstChildSize.height = firstChild.client_rect.bottom - client_rect.top
            //console.log('qwe')
        } else {
            //firstChildSize.width = firstChild.bounds.width //+ parseFloat(pickedStyles.paddingLeft)
            firstChildSize.width = firstChild.client_rect.right - client_rect.left
        }
    }

    // why is the child so small?
    if (firstChildSize.width < MIN_SIZE || firstChildSize.height < MIN_SIZE) {
        return null
    }
